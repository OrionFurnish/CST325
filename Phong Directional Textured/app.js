...
function updateAndRender() {
    ...
    if(appInput.right) {lightRotationY -= time.deltaTime*50;}
    if(appInput.left) {lightRotationY += time.deltaTime*50;}
    if(appInput.up) {lightRotationX -= time.deltaTime*50;}
    if(appInput.down) {lightRotationX += time.deltaTime*50;}
    var lightRotationMatrix = new Matrix4().makeRotationY(lightRotationY).multiply(new Matrix4().makeRotationX(lightRotationX));
    var lightDir = lightRotationMatrix.multiplyVector(new Vector4(lightDirection, 1.0));
